...
public class SougouScelFileParser {
    ...
    public SougouThesaurus readScelFile(String filePathName) throws IOException {
        ...
        thesaurus.setName(getNextNBytesAsString(length, bb));

        length = THESAURUS_CATEGORY_END - THESAURUS_CATEGORY_START;
        thesaurus.setCategory(getNextNBytesAsString(length, bb));

        length = THESAURUS_DESCRIPTION_END - THESAURUS_DESCRIPTION_START;
        thesaurus.setDescription(getNextNBytesAsString(length, bb));

        length = THESAURUS_EXAMPLE_END - THESAURUS_EXAMPLE_START;
        thesaurus.setExample(getNextNBytesAsString(length, bb));

        Map<Integer, String> pinyinTable = this.readPinYinTable(bb);
        if (pinyinTable != null) {
            thesaurus.setSwList(this.readWordList(bb, pinyinTable));
            sort(thesaurus.getSwList());
        }
    ...
    private Map<Integer, String> readPinYinTable(ByteBuffer bb) throws UnsupportedEncodingException {
    ...
    private List<SougouWord> readWordList(ByteBuffer bb, Map<Integer, String> globalPinyinTable)
    ...
    private int print2BytesToPositiveInteger(byte lowByte, byte highByte) {
